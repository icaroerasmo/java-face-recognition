...
- Startup requires valid Telegram settings. `BeansAndConfig` creates the `TelegramBot` bean eagerly and exits the process if `object-detection.telegram.bot-token` or `chat-id` is missing.
